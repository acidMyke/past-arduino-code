//========== ex2-2-1.ino ============
//========== Global Declaration ============
// pins 13, 12, 11 connected to 74HC138 inputs A, B, C
...
#define  speed  50  // define speed (times of repetition, switching time about 8ms*speed)
//========== Initialization ============
void setup()
{  pinMode(G1, OUTPUT);			// configure digital pin 10 as an output
      for(int i=0;i<3;i++) 		// configure digital pins 11 to 13 as outputs
	  pinMode(HC138[i], OUTPUT);
      for(int i=0;i<8;i++) 		// configure digital pins 2 to 9 as outputs
	  pinMode(Row[i], OUTPUT);
      digitalWrite(G1, 0);		// turn off LED matrix display (disable 74HC138)
} 
//========== Main Code ============
void loop()
{  for(int i=0;i<10;i++)		// display 10 characters
    {  for(int j=0;j<speed;j++)		// scan each character 50 times
	{  for(int k=0;k<8;k++)		// column scanning
	       ...
	       writeRow(myData[8*i+k]); // output display data to row pins
	       digitalWrite(G1,1);      // enable 74HC138 decoder
...
{  switch (x)			 
	...
	case 5:		// when x equals 5
	    ...
	    digitalWrite(HC138[0],1); // A=1
	    break; 	// exit
	case 6:		// when x equals 6
	    digitalWrite(HC138[2],1); // C=1
	    digitalWrite(HC138[1],1); // B=1
	    digitalWrite(HC138[0],0); // A=0
	    break; 	// exit
	case 7:		// when x equals 7
	    digitalWrite(HC138[2],1); // C=1
	    digitalWrite(HC138[1],1); // B=1
	    digitalWrite(HC138[0],1); // A=1
	    break; 	// exit
